...
def tts(key_value):
    ...
    tts_dict[keyname] = dict_value
    engine.say(tts_dict["Name"])
    engine.runAndWait()
...
def obj_detect(latest):
    ...
    detector.setModelPath(os.path.join(execution_path , "Weights/yolov3.pt"))
    detector.loadModel()
